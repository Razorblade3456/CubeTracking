...
function App() {
  ...
  const [editingNegatives, setEditingNegatives] = useState('0');
  ...
  const handleAdd = (event) => {
    ...
    const newItem = {
      ...
      text: trimmed,
      note: form.note.trim(),
      negatives: Math.max(0, Number.parseInt(form.negatives, 10) || 0),
      createdAt: Date.now(),
      completed: false,
      queued: false,
      completedAt: null
  ...
  const playCubeSmash = () => {
    ...
    if (pendingTargets.length === 0) {
      showToast('No armed bugs ready to zap yet.');
      return;
    }
  ...
  const startEdit = (item) => {
    ...
    setEditingNegatives(String(item.negatives ?? 0));
  };
  ...
  const saveEdit = (id) => {
    ...
    const negatives = Math.max(0, Number.parseInt(editingNegatives, 10) || 0);

    setItems((prev) =>
      prev.map((item) => (item.id === id ? { ...item, text: trimmed, note: trimmedNote, negatives } : item))
    );
    setEditingId(null);
    setEditingText('');
    setEditingNote('');
      ...
      <main className="list-section">
        <section className="todo-card">
          <div className="card-heading">
            <h2>Habits / To-Dos</h2>
            <p>Track missions, add optional notes, set negatives, and keep momentum with one-tap complete and undo.</p>
          </div>
            ...
            {activeItems.length === 0 ? (
              ...
              activeItems.map((item) => (
                <ItemRow
                  ...
                  editingNegatives={editingNegatives}
                  onEditTextChange={setEditingText}
                  onEditNoteChange={setEditingNote}
                  onEditNegativesChange={setEditingNegatives}
      ...
      <div className="item-main">
        {isEditing ? (
          <>
            ...
            />
            <input
              className="inline-editor inline-negative"
              type="number"
              min="0"
              value={editingNegatives}
              onChange={(event) => onEditNegativesChange(event.target.value)}
              placeholder="Negatives (0+)"
            />
          </>
        ) : (
          <>
            ...
            <small>
              ...
              {item.negatives > 0 ? ` · Negatives ${item.negatives}` : ''}
            </small>
          </>
        )}
      </div>
