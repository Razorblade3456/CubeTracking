...
function App() {
  ...
  const toggleComplete = (id) => {
    ...
    showToast(isCurrentlyComplete ? 'Cube restored.' : 'Cube smashed.');
  };
  ...
  return (
    <div className="app-shell">
      <header className="board-section">
        ...
        <div className="cube-board" aria-label="Enemy cube board">
          {items.length === 0 ? (
            ...
            items.map((item, index) => (
              <article
                key={item.id}
                className={`cube ${item.completed ? 'is-complete' : ''}`}
                style={{ '--delay': `${(index % 8) * 40}ms` }}
                title={item.text}
              >
                ...
                <p className="cube-label">{item.text}</p>
              </article>
            ))
          )}
        </div>
        ...
        <section className="todo-card">
          ...
          <div className="list-group completed">
            ...
            {completedItems.length === 0 ? (
              <p className="empty-state">Complete a task to smash a cube.</p>
            ) : (
